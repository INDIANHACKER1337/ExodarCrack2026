...
/**
 * HYPIXEL TEST MODE - Alternate Hypixel bypass variant
 *
 * WARNING: This mode sends packets directly and will flag on Vulcan/other ACs.
 * Only use on Hypixel servers.
 *
 * Based on Raven XD's HypixelTest autoblock mode.
 * Similar to Hypixel mode but with different packet order.
 */
public class HypixelTestMode extends BaseMode {
    ...
    public HypixelTestMode(AutoBlock parent) {
        super(parent);
    }
    ...
    public void onTick(EntityPlayerSP player, EntityLivingBase target, long now) {
        ...
        if (state == STATE_BLOCKING) {
            // First tick of blocking - send interact + block ONCE
            if (!isBlocking) {
                // HypixelTest sends interact BEFORE block (different from HypixelMode)
                if (lastTarget != null) {
                    sendInteractPackets(lastTarget);
                }
                sendBlockPacket();
    ...
    private void sendInteractPackets(EntityLivingBase target) {
        ...
        // Send in different order than HypixelMode
