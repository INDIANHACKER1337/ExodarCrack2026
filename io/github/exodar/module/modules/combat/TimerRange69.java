...
/**
 * TimerRange69 - Gothaj 3.3.0 TimerRange port
 *
 * Balance system:
 ...
 */
public class TimerRange69 extends Module {
    ...
    public TickSetting notInCombo = new TickSetting("Not In Combo", true);
    ...
    public TimerRange69() {
        super("TimerRange69", ModuleCategory.COMBAT);
    ...
    private boolean shouldStop() {
        ...
        if (onlyForward.isEnabled()) {
            if (getSpeed(player) <= 0.12 || !mc.gameSettings.keyBindForward.isKeyDown() ||
                predictedDistance > player.getDistanceToEntity(target) + 0.12) {
                return true;
            }
        }
    ...
    private boolean isTargetCloseOrVisible() {
        ...
        EntityPlayerSP player = getPlayer();
        if (player != null && player.getDistanceToEntity(target) <= 3.0) {
            return true;
        }
